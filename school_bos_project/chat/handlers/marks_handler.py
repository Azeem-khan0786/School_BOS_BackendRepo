...
def handle(phone, user, action, student):
    ...
    if not grades.exists():
        return get_lang_text("marks_no_data", lang).format(name=student.user.username)
    ...
    if action == "marks_recent":
        ...
        msg = f"{title}\n{get_lang_text('child_label', lang)}: {student.user.username}\n"
        for g in grades[:5]:
    ...
    # 🔸 Subject-wise Averages
        ...
        subjects = {}
        for g in grades:
            subjects.setdefault(g.subject.subject, []).append(g.marks_obtained)
    ...
    # 🔸 Overall Summary
        ...
        total = sum(g.max_marks for g in grades)
        obtained = sum(g.marks_obtained for g in grades)
