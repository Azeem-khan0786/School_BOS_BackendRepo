...
def handle(phone, user, action, student):
    ...
    exams = Exam.objects.filter(class_name__class_name=student.class_name).order_by("exam_date")
    if not exams.exists():
        return get_lang_text("exams_no_data", lang).format(name=student.user.username)
    ...
    if not exams.exists():
        return f"{get_lang_text('exams_no_data', lang)}"
