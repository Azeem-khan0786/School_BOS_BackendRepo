...
def handle(phone, user, student):
    ...
    all_att = Attendance.objects.filter(student=student.user)
    total = all_att.count()
    ...
    if fee:
        ...
        fee_line = get_lang_text("fees_no_data", lang).format(name=student.user.username)
    ...
    if grades:
        ...
        marks_lines = get_lang_text("marks_no_data", lang).format(name=student.user.username)
