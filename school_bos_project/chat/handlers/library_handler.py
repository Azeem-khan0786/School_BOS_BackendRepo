...
def handle(phone, user, action, student):
    ...
    issues = BookIssue.objects.filter(issued_to=student.user).order_by("-issue_date")
    if not issues.exists():
        return get_lang_text("library_no_data", lang).format(name=student.user.username)
    ...
    # 🔸 ACTION: Overdue Books
        ...
        if not due.exists():
            return get_lang_text("library_no_due_books", lang).format(name=student.user.username)
